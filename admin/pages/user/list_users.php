                ...
                <tbody>
                    ...
                    if ($row_count == 0) {
                        ...
                        while ($row_fetch_users = mysqli_fetch_array($get_user_result)) {
                            ...
                            $role = $row_fetch_users['role'];
                            $user_image = $row_fetch_users['image'];
                            $reg_no = $row_fetch_users['reg_no'];
                            $user_mobile = $row_fetch_users['phone'];
                            $department = $row_fetch_users['department'];
                            echo "
                            <tr>
                            <td>$id_number</td>
                            <td>$username</td>
                            <td>$role</td>
                            <td>
                                <img src='../uploads/images/users/$user_image' alt='$username photo' class='img-thumbnail' width='100px'/>
                            </td>
                            <td>$reg_no</td>
                            <td>$user_mobile</td>
                            <td>$department</td>
                            <td>
                                <a href='index.php?pages/delete_user=$user_id' data-bs-toggle='modal' data-bs-target='#deleteModal_$user_id'>
