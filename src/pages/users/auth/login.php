...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($action == 'login') {
        ...
        //     $error_user_id = "Admission ID is required for students!";
        // } elseif ($role != 'student'  && empty($user_id)) {
        //     $error_id = "ID Number is required for faculty!";
        // }
        ...
        if (empty($error_confirm_password) && empty($error_phone) && empty($error_user_id) && empty($error_id)) {
            ...
            $sql = "INSERT INTO users (username, password, phone, role, id, class, department, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";

            if ($stmt = $conn->prepare($sql)) {
                $stmt->bind_param("ssssssss", $username, $password, $phone, $role, $id, $class, $department, $user_id);
                if ($stmt->execute()) {
